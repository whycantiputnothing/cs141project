...
public class UI {
	...
	/**
	 * public method used to start the game. Prompts user if they want to start
	 * a new game, load a save file, or quit
	 */
	...
	/**
	 * main gameloop. Takes in the winning or losing condition for the while
	 * loop. Each iteration counts as a turn.
	 */
	 ...
	 * Asks for which direction the user would like to look and displays ninja
	 * ahead, the coast is clear, there is nothing to see there, or a room is
	 * blocking your view there is a built in loop so if the player chooses the
	 * wrong input the turn will not end
	 */
	...
	/**
	 * "2nd" set of choices. Prompts the user if they would like to move, shoot,
	 * or check ammo moving and shooting ends the player's turn while checking
	 * ammo does not. built in loop so if the user chooses a wrong input the
	 * turn doesn't end
	 */
	...
	/**
	 * prints out the prompts if move is selected. includes room checker for
	 * only being able to enter a room from the top
	 */
	private void move() {
	...
	/**
	 * prints dead message along with respawning the spy to the start of the
	 * game
	 */
	private void dead() {
	...
	private void powerUp() {
		...
		if (GE.getIsInvincible() && GE.getIsDebug() == false)
			System.out.println("You have " + (5 - (GE.getNumberOfMoves() - GE.getNumberOfMovesCounter()))
					+ " moves of invinicibility remaining");
		GE.resetPowerUpName();
	}

	/**
	 * prompts the user which direction they would like to shoot. displays a
	 * message if the user hit a ninja
	 */
	private void shoot() {
	...
	/**
	 * prints game over if the game was lost, or congradulations if the game was
	 * won. Prompts the user if they would like to go to main menu or quit
	 */
